...
void reset_fp_regs(u32 vmid, int vcpu_id)
{
	...
	el2_memcpy(&shadow_ctxt->gp_regs.fp_regs, &vcpu->arch.ctxt.fp_regs,
					sizeof(struct user_fpsimd_state));
}

//Management
static void get_crypt_buf(__u64 *buf,
...
static void put_crypt_buf(__u64 *buf,
		struct kvm_regs *kvm_regs)
{
...
static void encrypt_kvm_regs(u32 vmid,
		struct kvm_regs *kvm_regs)
{
...
static void decrypt_kvm_regs(u32 vmid, struct kvm_regs *kvm_regs)
{
	struct user_pt_regs *regs = &kvm_regs->regs;
	__u64 buf[8];
	__u64 out_buf[8];
...
void encrypt_gp_regs(u32 vmid, u32 vcpu_id)
{
	...
	el2_memcpy(&vcpu->arch.ctxt.regs, &gp_local.regs, sizeof(struct user_pt_regs));
	vcpu->arch.ctxt.sys_regs[SP_EL1] = gp_local.sp_el1;
	vcpu->arch.ctxt.sys_regs[ELR_EL1] = gp_local.elr_el1;
	vcpu->arch.ctxt.spsr_abt = gp_local.spsr[0];
	vcpu->arch.ctxt.spsr_und = gp_local.spsr[1];
	vcpu->arch.ctxt.spsr_irq = gp_local.spsr[2];
	vcpu->arch.ctxt.spsr_fiq = gp_local.spsr[3];
	el2_memcpy(&vcpu->arch.ctxt.fp_regs, &gp_local.fp_regs, sizeof(struct user_fpsimd_state));
	for (i = 0; i < 31; i++)
	...
	p = (uint64_t *)&vcpu->arch.ctxt.fp_regs;
	for (i = 0; i < 66; i++)
		printhex_ul(*p++);
}
...
void decrypt_gp_regs(u32 vmid, u32 vcpu_id)
{
	...
	uint64_t *p;
	shadow_ctxt = hypsec_vcpu_id_to_shadow_ctxt(vmid, vcpu_id);
	el2_memcpy(&gp_local.regs, &vcpu->arch.ctxt.regs, sizeof(struct user_pt_regs));
	gp_local.sp_el1 = vcpu->arch.ctxt.sys_regs[SP_EL1];
	gp_local.elr_el1 = vcpu->arch.ctxt.sys_regs[ELR_EL1];
	gp_local.spsr[0] = vcpu->arch.ctxt.spsr_abt;
	gp_local.spsr[1] = vcpu->arch.ctxt.spsr_und;
	gp_local.spsr[2] = vcpu->arch.ctxt.spsr_irq;
	gp_local.spsr[3] = vcpu->arch.ctxt.spsr_fiq;
	el2_memcpy(&gp_local.fp_regs, &vcpu->arch.ctxt.fp_regs, sizeof(struct user_fpsimd_state));
	...
	el2_memset(&vcpu->arch.ctxt.regs, 0, sizeof(struct user_pt_regs));
	vcpu->arch.ctxt.spsr_abt = 0;
	vcpu->arch.ctxt.spsr_und = 0;
	vcpu->arch.ctxt.spsr_irq = 0;
	vcpu->arch.ctxt.spsr_fiq = 0;
	el2_memset(&vcpu->arch.ctxt.fp_regs, 0, sizeof(struct user_fpsimd_state));
	for (i = 0; i < 31; i++)
		printhex_ul(shadow_ctxt->gp_regs.regs.regs[i]);
	printhex_ul(shadow_ctxt->gp_regs.regs.sp);
	printhex_ul(shadow_ctxt->gp_regs.regs.pc);
