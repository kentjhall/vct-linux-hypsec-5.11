...
{
    set_shadow_ctxt(ctxtid, EC, ec);
    if (ec == ARM_EXCEPTION_TRAP)
    {
        u64 hsr = get_shadow_ctxt(ctxtid, ESR_EL2);
...
void restore_shadow_kvm_regs(u32 ctxtid)
{
    ...
    {
        u64 ec = get_shadow_ctxt(ctxtid, EC);
        if (ec == ARM_EXCEPTION_TRAP)
        ...
        if (dirty & DIRTY_PC_FLAG) {
            ...
            set_shadow_ctxt(ctxtid, PC, pc + 4UL);
        }
        set_shadow_ctxt(ctxtid, DIRTY, 0UL);
        set_shadow_ctxt(ctxtid, FAR_EL2, 0UL);

        if (get_shadow_ctxt(ctxtid, FLAGS) & PENDING_FSC_FAULT)
        {
            post_handle_shadow_s2pt_fault(ctxtid);
        }

        set_shadow_ctxt(ctxtid, FLAGS, 0UL);
    }
}
