...
static int __hyp_text arm_smmu_id_size_to_bits(int size)
{
	switch (size) {
	case 0:
		return 32;
...
static int __hyp_text el2_smmu_cfg_probe(struct el2_arm_smmu_device *smmu)
{
	int err = -ENODEV;
	unsigned long size;
	u32 id;
	...
	/* We only support S2 translation now */
	if (!(id & ID0_S2TS))
		return err;
